...
export default class NutritionMealsFoodsDBService {
    ...
    static async multiAddFoods(mealId, foods, transaction) {
        if (!mealId || !foods || foods.length === 0) return [];
        ...
        try {
            ...
            for (const food of foods) {
                ...
                INSERT INTO dbo.MealLogsFoods (
                    MealLogId, OwnerId, CreatorId, CreatorName, USDAId, IsPublic, IsUSDA, Label, Category,
                    ServingUnit, OriginalServingSize, ServingSize, OriginalEnergyKcal, EnergyKcal,
                    OriginalCarbs, Carbs, OriginalProtein, Protein, OriginalFat, Fat, DominantMacro, AdditionalProps
                )
                OUTPUT INSERTED.Id
                VALUES (
                    @MealLogId, @OwnerId, @CreatorId, @CreatorName, @USDAId, @IsPublic, @IsUSDA, @Label, @Category,
            ...
            `;
                ...
                createdFoods.push({ ...food, id: newId, mealId });
            }

            return createdFoods;
        } catch (err) {
            console.error("multiAddFoods error:", err);
            throw err; // Let Database.runTransaction handle rollback
        }
    }
}

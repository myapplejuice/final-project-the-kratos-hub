...
export default function Meal({ title = 'Meal', num, onDeletePress = () => { }, onRenamePress = () => { }, onAddPress = () => { }, onFoodPress = () => { }, onAddPressVisible = true, onRenamePressVisible = true, onFoodPressVisible = true, onDeletePressVisible = true, foods = [] }) {
    ...
    return (
        <View style={styles.card}>
            {/* Header with arrow */}
            <TouchableOpacity onPress={() => setExpanded(prev => !prev)} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', zIndex: 9999 }}>
                <View>
                    <AppText style={{ fontSize: scaleFont(14), color: 'white' }}>Meal {num}</AppText>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                    {onRenamePressVisible && onDeletePressVisible && (
                        <View style={{ flexDirection: 'row' }}>
                            <TouchableOpacity style={{ backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 10, alignItems: 'center', justifyContent: 'center', marginEnd: 10, paddingVertical: 8, paddingHorizontal: 10 }} onPress={onRenamePress}>
                                <Image style={{ width: 16, height: 16, tintColor: 'white' }} source={Images.edit} />
                            </TouchableOpacity>
                            <TouchableOpacity style={{ backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 10, alignItems: 'center', justifyContent: 'center', paddingVertical: 8, paddingHorizontal: 10 }} onPress={onDeletePress}>
            ...
            {foods.length > 0 ? (
                foods.map((food, index) => (
                    <TouchableOpacity disabled={!onFoodPressVisible} onPress={onFoodPress} key={index} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginVertical: 12 }}>
                        <View>
                            <AppText style={{ color: 'white', fontSize: scaleFont(13) }}>{food.name}</AppText>
                            <AppText style={{ color: colors.mutedText, fontSize: scaleFont(11) }}>{food.creator}, {food.amount} {food.unit || 'g'}</AppText>
                        </View>
            ...
            <View style={{ padding: 15, alignItems: 'center', backgroundColor: 'rgba(58,58,58,0.49)', borderRadius: 15, marginTop: foods.length > 0 ? 7 : 15, marginBottom: onAddPressVisible ? 0 : 15 }}>
                <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                    <AppText style={{ color: nutritionColors.energy1, fontSize: scaleFont(10) }}>{convertEnergy(totals.energy, 'kcal', user.preferences.energyUnit.key)} {user.preferences.energyUnit.field}</AppText>
